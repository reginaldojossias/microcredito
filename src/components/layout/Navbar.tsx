...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 h-[72px] border-b border-line/80 bg-surface/90 backdrop-blur-md">
      <div className="mx-auto flex h-full max-w-[1200px] items-center justify-between px-5">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div className="leading-none">
            <div className="text-[15px] font-bold tracking-[-0.04em] text-ink">KUKULA</div>
            <div className="mt-1 text-[10px] uppercase tracking-[0.12em] text-gold-2">
              {dict.common.brand}
            </div>
          </div>
        </Link>
        ...
        {isHome ? (
          ...
          <div className="hidden text-[12px] text-ink-secondary md:block">
            {dict.common.platformTagline}
          </div>
        )}
...
export function Footer() {
  ...
  return (
    <footer className="border-t border-line bg-surface">
      <div className="mx-auto flex max-w-[1200px] flex-col gap-3 px-5 py-8 text-[11px] text-ink-tertiary md:flex-row md:items-center md:justify-between">
        <div className="font-medium text-ink-secondary">
          Kukula {dict.common.brand}
        </div>
        <div>
          © {new Date().getFullYear()} · {dict.footer.copyright}
        </div>
        <div className={cn("flex gap-4")}>
          <Link href="/login" className="hover:text-gold-2">
            {dict.nav.clientArea}
          </Link>
          <Link href="/admin" className="hover:text-gold-2">
            {dict.nav.admin}
          </Link>
        </div>
      </div>
