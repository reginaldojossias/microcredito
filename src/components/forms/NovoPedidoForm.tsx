  ...
  if (result) {
    return (
      <Card className="max-w-xl">
        <div className="text-[11px] uppercase tracking-[0.08em] text-ink-tertiary">Referência</div>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-5">
      <Card className="lg:col-span-3">
        ...
        >
          <div>
            <label className="mb-1.5 block text-[12px] text-ink-secondary">Produto</label>
            <select
              value={productId}
              onChange={(e) => {
          ...
          <div>
            <label className="mb-1.5 block text-[12px] text-ink-secondary">
          ...
          <div>
            <label className="mb-1.5 block text-[12px] text-ink-secondary">Finalidade</label>
            <input
              value={purpose}
      ...
      <Card className="lg:col-span-2">
        <div className="text-[11px] uppercase tracking-[0.08em] text-ink-tertiary">Simulação</div>
        <h3 className="mt-2 text-[22px] font-bold tracking-[-0.04em]">{product.name}</h3>
        <div className="mt-6 space-y-4">
          <div className="flex justify-between text-[14px]">
            <span className="text-ink-secondary">Montante</span>
            <span className="font-semibold">{formatCurrency(amount)}</span>
          </div>
          <div className="flex justify-between text-[14px]">
            <span className="text-ink-secondary">Prestação mensal</span>
            <span className="font-semibold">
              {formatCurrency(simulation.monthlyPayment)}
            </span>
          </div>
          <div className="flex justify-between text-[14px]">
            <span className="text-ink-secondary">Total a pagar</span>
            <span className="font-semibold">
              {formatCurrency(simulation.totalPayable)}
            </span>
          </div>
          <div className="flex justify-between text-[14px]">
            <span className="text-ink-secondary">Juros totais</span>
            <span className="font-semibold">
